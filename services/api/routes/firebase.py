...
async def add_pocket_article(
    ...
    try:
        ...
        from common.storage.database_storage import ArticleMetadata
        ...
        # Trigger archival via Celery
        from common.celery_config import celery_app, ARCHIVER_TASK_MAP

        tasks_triggered = 0
        if data.archiver == "all":
            for _, task_name in ARCHIVER_TASK_MAP.items():
                celery_app.send_task(task_name, args=[data.url, item_id])
                tasks_triggered += 1
        else:
            task_name = ARCHIVER_TASK_MAP.get(data.archiver)
            if task_name:
                celery_app.send_task(task_name, args=[data.url, item_id])
                tasks_triggered += 1
            else:
                logger.warning(f"Unknown archiver: {data.archiver}")
        ...
        if tasks_triggered > 0:
            ...
            logger.warning("No tasks triggered for archival")
            return AddPocketArticleResponse(
                article_id=item_id,
                status="saved",
                message="Article saved to database but archival not queued (no valid archiver)"
            )
    ...
    try:
        ...
        for upload in storage_uploads:
            if upload.get('success') and upload.get('storage_uri'):
                # Extract GCS path from common.storage URI
    ...
    try:
        ...
        if tasks_triggered > 0:
            ...
            return AddPocketArticleResponse(
                article_id=item_id,
                status="saved",
                message="Article saved but not queued (no valid archiver)"
            )
...
async def archive_article(
    ...
    try:
        ...
        # Queue archival task via Celery
        from common.celery_config import celery_app, ARCHIVER_TASK_MAP
        ...
        if data.archiver == "all":
            for _, task_name in ARCHIVER_TASK_MAP.items():
                celery_app.send_task(task_name, args=[data.url, data.item_id])
                tasks_triggered += 1
        else:
            task_name = ARCHIVER_TASK_MAP.get(data.archiver)
            if task_name:
        ...
        logger.info(
            ...
            extra={"item_id": data.item_id, "archiver": data.archiver, "tasks_queued": tasks_triggered}
        )
